...
import type { SearchResult, UseSearchFunctionalityProps } from './types';

const SEARCH_LIMIT = 4

export function useSearchFunctionality({ searchableData }: UseSearchFunctionalityProps) {
  ...
  // Search function with intelligent ranking - memoized
  const performSearch = useCallback((query: string): SearchResult[] => {
    ...
    return sortedResults.slice(0, SEARCH_LIMIT); // Limit to 20 results for performance
  }, [searchableData]);

  // Get all items in proper order (unit -> location -> customer) - memoized
